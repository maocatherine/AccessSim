        ...
        // implements ReachableZonesFilter
{
    ...
    public Map<Zone, Set<Mode>> filter(
        ...
        // TODO: prüfen, ob man das braucht (check if you need it)
        ...
        for (Zone destination : zones) {
            for (Mode mode : availableModes) {
                ...
                double time = this.impedance.getTravelTime(currentZone.getId(), destination.getId(), mode, date)
                        + this.impedance.getTravelTime(destination.getId(), nextFixedZone.getId(), mode, date);
