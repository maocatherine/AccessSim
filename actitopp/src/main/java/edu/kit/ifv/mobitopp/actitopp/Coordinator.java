    ...
    private void executeStep3(String id) {
        for (HDay currentDay : pattern.getDays()) {
            ...
            if (Configuration.coordinated_modelling) {
                ...
                if (maxnumberoftours != -1)
                    step.limitUpperBoundOnly((maxnumberoftours >= minnumberoftours ? maxnumberoftours : minnumberoftours));
            }
            ...
            if (id.equals("3B"))
                assert (currentDay.getAmountOfTours() >= numberoftoursperday_lowerboundduetojointactions[currentDay.getIndex()]) : "wrong number of tours - violating lower bound due to joint actions";
        }
    }
        ...
        for (HDay currentDay : pattern.getDays()) {
            ...
            for (int i = currentDay.getLowestTourIndex(); i <= currentDay.getHighestTourIndex(); i++) {
                ...
                if (currentDay.getTotalAmountOfActivitites() < numberofactsperday_lowerboundduetojointactions[currentDay.getIndex()]) {
                    ...
                    if (id.equals("5B") && currentTour.getIndex() == currentDay.getHighestTourIndex())
                        minimumnumberofactivities = remainingnumberofactivities;
                }
                ...
                for (int j = 1; j <= step.getDecision(); j++) {
                    ...
                    if (id.equals("5A") && !currentDay.existsActivity(currentTour.getIndex(), -1 * j))
                        act = new HActivity(currentTour, (-1) * j);
                    // 5B - activity after main activity
                    if (id.equals("5B") && !currentDay.existsActivity(currentTour.getIndex(), +1 * j))
                        act = new HActivity(currentTour, (+1) * j);
            ...
            if (id.equals("5B"))
                assert (currentDay.getTotalAmountOfActivitites() >= numberofactsperday_lowerboundduetojointactions[currentDay.getIndex()]) : "wrong number of activities - violating lower bound due to joint actions";
        }
    }
        ...
        for (int indexinliste = 0; indexinliste < listjointact.size(); indexinliste++) {
             ...
             */
            {
                ...
                for (HActivity act : possibleact) {
                    if ((act.getAttributefromMap("actreplacedbyjointact") != null ? act.getAttributefromMap("actreplacedbyjointact") : 0) == 1.0)
                        lastactreplaced = act;
                }
    ...
    private void executeStep8_MainAct(String id_dc, String id_wrd) throws InvalidPatternException {
        ...
        for (HDay currentDay : pattern.getDays()) {
            ...
            for (HTour currentTour : currentDay.getTours()) {
                ...
                if (id_dc.equals("8B") && currentTour.isFirstTouroftheDay())
                    running = true;  // 8B for first tour of the day
                if (id_dc.equals("8D") && !currentTour.isFirstTouroftheDay())
                    running = true;    // 8D for all other tours
                ...
                if (running) {
                    ...
                    if (!currentActivity.durationisScheduled()) {
                        ...
                        if (loc_lowerbound == loc_upperbound) {
                            ...
                            if (loc_upperbound <= step_dc.getUpperBound() || step_dc.getUpperBound() == -1)
                                step_dc.limitUpperBoundOnly(loc_upperbound);
                            ...
                            if (step_dc.getLowerBound() >= step_dc.getUpperBound())
                                step_dc.limitLowerBoundOnly(step_dc.getUpperBound());
                        }
            ...
            if (!currentTour.isScheduled()) {
                ...
                if (Configuration.coordinated_modelling) {
                    if (currentTour.existsAttributeinMap("default_start_cat_yes") && currentTour.getAttributefromMap("default_start_cat_yes") == 1.0d) {
                        ...
                        if (defaultcat >= lowerbound && defaultcat <= upperbound)
                            step_dc.limitUpperandLowerBound(defaultcat, defaultcat);
                    }
                }
...
//TODO previousTour needs to be scheduled if modeling is chronological 
                ...
                if (previousTour != null && previousTour.isScheduled())
                    assert currentTour.getStartTimeWeekContext() > previousTour.getEndTimeWeekContext() : "Tours are overlapping!";
            }
        }
    }
    ...
    private int[] calculateDurationBoundsDueToOtherActivities(HActivity act) throws InvalidPatternException {
        ...
        for (HActivity tmpact : dayofact.getAllActivitiesoftheDay()) {
            ...
            if (act.compareTo(tmpact) == -1) {
                ...
                if (tmpact.startTimeisScheduled() && (last_act_scheduled == null || tmpact.getStartTime() > last_act_scheduled.getStartTime()))
                    last_act_scheduled = tmpact;
            }

            // Search for later activity with determined starting time
            if (act.compareTo(tmpact) == +1) {
                //System.out.println(tmpact.getTour().getIndex() + "/" + tmpact.getIndex());
                if (tmpact.startTimeisScheduled() && (next_act_scheduled == null || tmpact.getStartTime() < next_act_scheduled.getStartTime()))
                    next_act_scheduled = tmpact;
            }
        }
    ...
    private void createHomeActivities(List<HActivity> allmodeledActivities) throws InvalidPatternException {
        ...
        if (allmodeledActivities.size() != 0) {
            ...
            if (duration1 <= 0)
                throw new InvalidPatternException("person", this.pattern, "person error - no home activity possible at beginning of the week!");
            ...
            for (int i = 0; i < allmodeledActivities.size() - 1; i++) {
                ...
                if (act.isActivityLastinTour()) {
                    ...
                    if (duration2 <= 0)
                        throw new InvalidPatternException("person", this.pattern, "person error - no home activity possible after end of the tour! - " + start_next_tour + " // " + ende_tour);
